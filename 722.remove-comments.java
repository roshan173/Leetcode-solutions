...
class Solution {
    public List<String> removeComments(String[] source) {
        ...
        for (String str: source) {
            if (!multiLine) sb = new StringBuilder();
            int n = str.length();
            for (int i=0; i<n; i++) {
                if (!multiLine && i+1 < n && str.charAt(i) == '/' && str.charAt(i + 1) == '/') break;
                if (!multiLine && i+1 < n && str.charAt(i) == '/' && str.charAt(i + 1) == '*') {
                    multiLine = true;
                    i++;
                }
                else if (multiLine && i+1 < n && str.charAt(i) == '*' && str.charAt(i+1) == '/') {
                    multiLine = false;
                    i++;
                }
                else if (!multiLine) sb.append(str.charAt(i));
            }
